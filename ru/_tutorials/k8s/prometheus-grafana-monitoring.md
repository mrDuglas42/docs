...
1. Создайте конфигурационный файл `trickster.yaml`, содержащий настройки trickster:
   ...
   Вы можете изменить размер хранилища, отведенного для работы кеширующего прокси. Укажите нужное значение размера хранилища в параметре `PersistentVolumeClaim:spec:resource:requests:storage`.
...
1. [Удалите диск, который был создан](https://yandex.cloud/ru/docs/compute/operations/disk-control/delete) он имеет метку, которую можно посмотреть через `kubectl describe pvc trickster-pvc`, она отображается в поле `Volume:` и имеет вид `pvc-********-****-****-****-************`
